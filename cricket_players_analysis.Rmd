...
```
##Introduction
The analysis was conducted as a part of recruitment process for SAI Global. To perform it I used 3 libraries listed below.
```{r libraries, warning=FALSE, echo=FALSE, message=FALSE}
library(data.table)
library(tidyverse)
library(kableExtra)
```
Full specification of the software used is listed on the bottom of the document. To begin with, I started with unzipping the file received and loading the data into RStudio as characters to avoid unnecessary factorization. 
